...
fh32_permute_string(const uint8_t *v, size_t len, uint32_t seed)
{
	...
	case 1: case 2: case 3:
		//st = fh32_permute(fh_load_u24(v, len), 0, st);
		st.a = fh_load_u24(v, len);
		goto mix4;
		//st.c ^= st.a; st.c -= FH_ROTL(st.a, 19);
		//st.d ^= st.a; st.d -= FH_ROTL(st.a, 16);
		//break;
	case 4:
		st.a = fh_load_u32(v);
mix4:
	...
	case 5: case 6: case 7:
		st.a = fh_load_u32(v);
		st.b = fh_load_u24(v+4,len&3);
		st.c ^= st.a; st.c -= FH_ROTL(st.b, 19);
		st.d ^= st.b; st.d -= FH_ROTL(st.a, 10);
		break;
	case 8:
		st.a = fh_load_u32(v);
		//st.b = fh_load_u32(v+len-4);
		st.b = fh_load_u32(v+4);
		st.c ^= st.a; st.c -= FH_ROTL(st.b, 19);
		st.d ^= st.b; st.d -= FH_ROTL(st.a, 10);
		break;
	...
	case 0: break;
	case 1: case 2: case 3: {
		uint32_t m = fh_load_u24(v, len);
		st.a ^= m;
		st.b ^= m;
		break;}
	case 4: {
		uint32_t m = fh_load_u32(v);
		st.a ^= m;
		st.b ^= m;
		break; }
	case 5: case 6: case 7:
	...
	st.c ^= st.b; st.c -= FH_ROTL(st.a, 19);
	st.d ^= st.a; st.d -= FH_ROTL(st.b, 20);
#endif
	uint32_t t = seed ^ l;
#define rot FH_ROTL
#define final2(a,b,c) \
